    ...
    <div class="storefront-container">
        ...
        <div class="mb-5">
            <a href="{{ route('store.cart') }}"
               class="group text-xs font-bold text-slate-500 hover:text-slate-900 transition-colors uppercase tracking-wider flex items-center gap-1.5"
               style="text-decoration:none">
                <i class="fa-solid fa-arrow-left-long transition-transform group-hover:-translate-x-1"></i> Back to
                Shopping Cart
            </a>
        </div>
                    ...
                    <div class="bg-white border border-[#e8e4df] rounded-2xl p-5 md:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-2.5" id="addressCardsGrid">
                            @foreach($addresses as $addr)
                                <div
                                    class="border rounded-xl p-5 relative cursor-pointer transition-all hover:border-slate-400 address-card-item {{ $addr->is_default ? 'border-slate-900 bg-slate-50/10' : 'border-[#e8e4df]' }}"
                                    onclick="selectAddress(this, {{ $addr->id }})" id="address-card-{{ $addr->id }}">

                                <span
                                    class="select-check-icon absolute top-4 right-4 text-accent text-lg {{ $addr->is_default ? 'block' : 'hidden' }}">
                                    ...
                                    <div
                                        class="text-xs text-slate-500 leading-relaxed address-details-text">{{ $addr->address }}
                                        <br/>{{ $addr->city }}, {{ $addr->state }} {{ $addr->zip }}
                                        <br/>{{ $addr->country }}</div>

                                    <input type="radio" name="address_id" value="{{ $addr->id }}"
                                           class="hidden" {{ $addr->is_default ? 'checked' : '' }}/>

                                    <div class="flex gap-2.5 mt-3 items-center justify-end">
                                        <span
                                            class="text-[10px] font-bold text-accent cursor-pointer hover:text-slate-900 transition-colors select-none edit-link"
                                            onclick="event.stopPropagation(); openCheckoutEditAddressModal({{ json_encode($addr) }})"><i
                                                class="fa-regular fa-pen-to-square mr-1"></i>Edit</span>
                                        <span class="text-slate-200 select-none">•</span>
                                        <span
                                            class="text-[10px] font-bold text-rose-500 hover:text-rose-700 cursor-pointer select-none delete-link"
                                            onclick="event.stopPropagation(); deleteCheckoutAddress({{ $addr->id }})"><i
                                                class="fa-regular fa-trash-can mr-1"></i>Delete</span>
                                    </div>
                                </div>
                            @endforeach
                            ...
                            <div
                                class="border-2 border-dashed border-[#e8e4df] hover:border-slate-400 hover:bg-slate-50/50 rounded-xl p-5 flex flex-col items-center justify-center cursor-pointer gap-2 min-h-[140px] transition-all {{ $addresses->isEmpty() ? 'col-span-3' : '' }}"
                                onclick="openCheckoutAddressModal()">
                            ...
                            @if(($selectionMode ?? 'user_choice') === 'user_choice')
                                <div class="flex flex-col gap-3">
                                    @forelse($shippingRates as $rate)
                                        <div
                                            class="border rounded-xl p-4 flex items-center justify-between cursor-pointer hover:border-slate-400 transition-all shipping-rate-item {{ $loop->first ? 'border-slate-900 bg-slate-50/10' : 'border-[#e8e4df]' }}"
                                            onclick="selectShippingRate(this, {{ $rate->id }}, {{ $rate->calculated_cost }})">
                                            <div class="flex items-center gap-3">
                                                <input type="radio" name="shipping_rate_id" value="{{ $rate->id }}"
                                                       {{ $loop->first ? 'checked' : '' }} class="text-slate-900 focus:ring-slate-900 border-slate-300">
                                                <div>
                                                    <p class="text-sm font-semibold text-slate-800">{{ $rate->name }}@if(($rate->type ?? 'flat') === 'percent')
                                                            ({{ number_format($rate->cost, 1) }}%)
                                                        @endif</p>
                                                    @if($rate->min_order_amount > 0)
                                                        <p class="text-[10px] text-slate-400">Min. order value:
                                                            ₹{{ number_format($rate->min_order_amount, 2) }}</p>
                                                    @endif
                                                </div>
                                            </div>
                                            <span
                                                class="text-sm font-bold text-slate-900">₹{{ number_format($rate->calculated_cost, 2) }}</span>
                                        </div>
                                    @empty
                                        <div class="text-xs text-slate-400 py-2">
                                            <i class="fa-solid fa-circle-info mr-1 text-slate-400"></i> No shipping
                                            charges registered. Standard free shipping is applied.
                                        </div>
                                    @endforelse
                                </div>
                            @else
                    ...
                    <div class="bg-white border border-[#e8e4df] rounded-2xl p-5 md:p-6">
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    @forelse($enabledGateways as $gateway)
                                        ...
                                        @endphp
                                        <div
                                            class="border border-slate-200 rounded-xl px-4 py-3 flex items-center justify-between cursor-pointer hover:border-slate-350 transition-all payment-method-item h-14 {{ $isFirst ? $activeClass : 'bg-white' }}"
                                            ...
                                            <div class="flex items-center gap-2.5 min-w-0">
                                                <input type="radio" name="payment_method"
                                                       ...
                                                       {{ $isFirst ? 'checked' : '' }} class="text-slate-900 focus:ring-slate-900 border-slate-300 shrink-0">
                                            ...
                                            <div class="shrink-0 flex items-center">
                                                @if($gateway->getIdentifier() === 'cod')
                                                    <svg class="h-4.5 text-slate-450" viewBox="0 0 24 24" fill="none"
                                                ...
                                                @elseif($gateway->getIdentifier() === 'razorpay')
                                                    <svg class="h-4.5 text-[#0c66ff]" viewBox="0 0 24 24" fill="none"
                                    ...
                                    @empty
                                        <div class="col-span-full text-xs text-slate-400 py-2">
                                            <i class="fa-solid fa-circle-info mr-1 text-slate-400"></i> No payment
                                            methods enabled. Please contact support.
                                        </div>
                                    @endforelse
                                </div>
                                ...
                                <div class="mt-2 flex flex-col gap-3">
                                    @foreach($enabledGateways as $gateway)
                                        <div class="gateway-fields-wrapper {{ $loop->first ? '' : 'hidden' }}"
                                             id="fields-{{ $gateway->getIdentifier() }}">
                        ...
                        @else
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="card_name" required value="{{ old('card_name') }}" class="inp"
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="card_num" required value="{{ old('card_num') }}" class="inp"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="card_expiry" required value="{{ old('card_expiry') }}"
                                           class="inp" placeholder="MM/YY"/>
                                    @error('card_expiry') <p
                                        class="text-rose-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
            ...
            <div class="bg-white border border-[#e8e4df] rounded-2xl p-5 md:p-6">
                ...
                <div class="flex flex-col mb-5">
                    @foreach($cart as $item)
                        <div class="flex items-center gap-3 py-3 border-b border-[#e8e4df] last:border-b-0">
                            <div
                                class="w-[50px] h-[66px] rounded-lg overflow-hidden bg-[#f0ece7] shrink-0 border border-[#e8e4df]">
                                <img src="{{ $item['img'] }}" alt="{{ $item['name'] }}"
                                     class="w-full h-full object-cover"/>
                            </div>
                            <div class="flex-1 min-w-0 text-xs">
                                <p class="font-semibold text-slate-800 truncate">{{ $item['name'] }}</p>
                                <p class="text-slate-400 mt-0.5">Qty: {{ $item['quantity'] }} @if($item['size'])
                                        · Sz: {{ $item['size'] }}
                                    @endif</p>
                            </div>
                            <span
                                class="font-semibold text-xs text-slate-900 ml-2">₹{{ number_format($item['price'] * $item['quantity'], 2) }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="flex flex-col gap-2.5 text-xs border-t border-slate-100 pt-4">
                    <div class="flex justify-between text-slate-500">
                        <span>Cart Subtotal</span><span>₹{{ number_format($subtotal, 2) }}</span></div>
                    @if($discount > 0)
                        <div class="flex justify-between text-emerald-600 font-semibold">
                            <span>Promo Discount</span><span>-₹{{ number_format($discount, 2) }}</span></div>
                    @endif
                    @if($taxLabel)
                        <div class="flex justify-between text-slate-500">
                            <span>{{ $taxLabel }}</span><span>₹{{ number_format($tax, 2) }}</span></div>
                    @endif
                    @if($hasShippingPackage)
                        <div class="flex justify-between text-slate-500">
                            <span>Shipping</span>
                            <span id="shipping-charge-display"
                                  class="{{ $shippingCost > 0 ? 'text-slate-900 font-semibold' : 'text-emerald-600 font-semibold' }}">
                        {{ $shippingCost > 0 ? '₹' . number_format($shippingCost, 2) : 'Free' }}
                    </span>
                        </div>
                    @endif
                    <div
                        class="flex justify-between font-bold text-slate-900 text-sm border-t border-slate-100 pt-3 mt-1">
