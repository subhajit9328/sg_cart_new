                ...
                <form action="{{ route('store.account.profile.update') }}" method="POST" class="w-full flex flex-col gap-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input name="last_name" id="last_name" required maxlength="255" class="w-full px-3.5 py-2.5 border border-[#e8e4df] dark:border-[#2e2c28] rounded-lg text-sm text-slate-900 dark:text-slate-100 bg-white dark:bg-[#1a1916] outline-none transition-all shadow-[inset_0_1px_2px_rgba(0,0,0,0.01)] focus:border-slate-900 focus:dark:border-accent focus:ring-3 focus:ring-slate-900/5 focus:dark:ring-accent/10" value="{{ auth('customer')->user() ? (explode(' ', auth('customer')->user()->name)[1] ?? '') : 'Doe' }}"/>
        ...
        if (profileForm) {
            ...
            function validateFirstName() {
                ...
                if (!val) {
                    ...
                    if (errFirstName) {
                        errFirstName.textContent = 'First name must not exceed 255 characters.';
                        errFirstName.classList.remove('hidden');
                    }
                    firstNameInput.classList.add('border-rose-500');
                    return false;
                } else if (!/^[a-zA-ZáéíóúýÁÉÍÓÚÝćĆńŃśŚźŹĺĹŕŔ\s\-\']+$/.test(val)) {
                    if (errFirstName) {
                        errFirstName.textContent = 'First name contains invalid characters.';
                        errFirstName.classList.remove('hidden');
                    }
                    firstNameInput.classList.add('border-rose-500');
                    return false;
            ...
            function validateLastName() {
                ...
                if (!val) {
                    ...
                    if (errLastName) {
                        errLastName.textContent = 'Last name must not exceed 255 characters.';
                        errLastName.classList.remove('hidden');
                    }
                    lastNameInput.classList.add('border-rose-500');
                    return false;
                    ...
                    if (errLastName) {
                        errLastName.textContent = 'Last name contains invalid characters.';
                        errLastName.classList.remove('hidden');
                    }
                    lastNameInput.classList.add('border-rose-500');
                    return false;
            ...
            function validatePhone() {
                ...
                if (!val.startsWith('+')) {
                    if (errPhone) {
                        errPhone.textContent = 'The phone number must include a country code starting with +.';
                ...
                if (!/^\d+$/.test(digits)) {
                    if (errPhone) {
                        errPhone.textContent = 'The phone number must contain only digits after the + country code.';
                        errPhone.classList.remove('hidden');
                    }
                    phoneInput.classList.add('border-rose-500');
                    return false;
                ...
                if (digits.length < 7 || digits.length > 15) {
                    if (errPhone) {
                        if (digits.length < 7) {
                            errPhone.textContent = 'The phone number must be at least 7 digits.';
                        } else {
                            errPhone.textContent = 'The phone number must not be more than 15 digits.';
                        }
                        errPhone.classList.remove('hidden');
                    }
                    phoneInput.classList.add('border-rose-500');
