...
@section('title', 'Edit Category — SGCart Admin')
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm w-full overflow-hidden">
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category->ulid) }}" enctype="multipart/form-data" class="p-6 space-y-5">
        @csrf
        @method('PUT')

        <!-- Name Field -->
        <div>
            <label for="name" class="block text-slate-700 dark:text-slate-300 text-sm font-semibold mb-2">Category Name *</label>
        ...
        <div class="space-y-3">
            ...
            @if($category->image)
                <div class="flex items-center gap-3 p-3 border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 rounded-xl w-fit">
                    ...
                    <div>
                        <p class="text-xs font-semibold text-slate-700 dark:text-slate-300">Current Image</p>
                        <p class="text-[10px] text-slate-400 mt-0.5">Uploading a new file will replace this image.</p>
                    </div>
                </div>
            @endif
