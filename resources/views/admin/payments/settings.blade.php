    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($gateways as $gateway)
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs overflow-hidden flex flex-col justify-between transition-all duration-300 hover:shadow-md">
                <!-- Gateway Header -->
                <div class="p-4 border-b border-slate-100 dark:border-slate-850 bg-slate-50/20 dark:bg-slate-900/10 flex items-center justify-between gap-3">
                    <div class="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-wrap items-center gap-1.5">
                                ...
                                <span class="font-mono text-[8px] font-bold text-slate-400 dark:text-slate-550 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded border border-slate-150 dark:border-slate-700/60 uppercase tracking-wider">
                                    {{ $gateway->getId() }}
                                </span>
                            </div>
                        </div>
                ...
                <form action="{{ route('admin.payments.settings.update') }}" method="POST" id="config-form-{{ $gateway->getId() }}" class="p-5 flex-1 flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex flex-col gap-3">
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-550 mb-1.5 uppercase tracking-wider">Display Name</label>
                                <input type="text" name="settings[{{ $gateway->getId() }}][name]" value="{{ $dbMethod->name ?? $gateway->getName() }}" class="w-full bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-slate-800 dark:text-slate-100 transition-all shadow-2xs" required>
                            </div>
                            
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-550 mb-1.5 uppercase tracking-wider">Description</label>
                                <textarea name="settings[{{ $gateway->getId() }}][description]" rows="2" class="w-full bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-slate-800 dark:text-slate-100 transition-all shadow-2xs resize-none" required>{{ $dbMethod->description ?? $gateway->getDescription() }}</textarea>
                            </div>
                        </div>
                        ...
                        @if(count($gateway->getConfigSchema()) > 0)
                            <div class="border-t border-slate-100 dark:border-slate-800 pt-4 mt-2">
                                <div class="flex items-center gap-1.5 mb-3">
                                    <i class="fa-solid fa-key text-[10px] text-slate-450 dark:text-slate-500"></i>
                                    <span class="text-[10px] font-bold text-slate-400 dark:text-slate-550 uppercase tracking-wider">API Credentials</span>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    @foreach($gateway->getConfigSchema() as $key => $field)
                                        ...
                                        @endphp
                                        <div>
                                            <label class="block text-[9px] font-bold text-slate-500 dark:text-slate-450 mb-1.5 uppercase tracking-wider">
                                                {{ $field['label'] }} @if($isRequired)<span class="text-rose-500">*</span>@endif
                                            </label>
                                            @if($field['type'] === 'textarea')
                                                <textarea name="settings[{{ $gateway->getId() }}][config][{{ $key }}]" rows="2" class="w-full bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-slate-800 dark:text-slate-100 shadow-2xs resize-none" {{ $isRequired ? 'required' : '' }}>{{ $value }}</textarea>
                                            @elseif($field['type'] === 'select')
                                                <select name="settings[{{ $gateway->getId() }}][config][{{ $key }}]" class="w-full bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-slate-800 dark:text-slate-100 shadow-2xs" {{ $isRequired ? 'required' : '' }}>
                                                    @foreach($field['options'] as $optVal => $optLabel)
                                                        <option value="{{ $optVal }}" {{ $value == $optVal ? 'selected' : '' }}>{{ $optLabel }}</option>
                                                    @endforeach
                                                </select>
                                            @else
                                                <input type="{{ $field['type'] }}" name="settings[{{ $gateway->getId() }}][config][{{ $key }}]" value="{{ $value }}" class="w-full bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-slate-800 dark:text-slate-100 shadow-2xs" {{ $isRequired ? 'required' : '' }}>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                ...
                <div class="px-5 pb-5 pt-3 border-t border-slate-100 dark:border-slate-850 flex items-center gap-3">
                    ...
                    <form action="{{ route('admin.payments.settings.update') }}" method="POST" class="flex-1">
                        ...
                        <button type="submit" class="w-full py-2.5 rounded-xl text-xs font-semibold transition-all border shadow-2xs flex items-center justify-center gap-1.5
                            @if($isEnabled)
                                bg-rose-50 hover:bg-rose-100/80 border-rose-200 text-rose-600 dark:bg-rose-950/20 dark:hover:bg-rose-955/30 dark:border-rose-900/40 dark:text-rose-455
                            @else
                                bg-emerald-50 hover:bg-emerald-100/80 border-emerald-200 text-emerald-600 dark:bg-emerald-950/20 dark:hover:bg-emerald-955/30 dark:border-emerald-900/40 dark:text-emerald-400
                            @endif">
                            @if($isEnabled)
        ...
        @empty
            <div class="col-span-full bg-white dark:bg-slate-900 border border-slate-205 dark:border-slate-800 rounded-2xl p-12 text-center text-slate-400">
