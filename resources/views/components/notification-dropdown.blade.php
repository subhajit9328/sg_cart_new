...
<div class="relative inline-block text-left z-100!" id="notificationDropdownContainer">
    <!-- Trigger Button -->
    <button type="button" id="notificationBellBtn" @class([
    'relative py-2 px-3 transition-all cursor-pointer border-none outline-none bg-transparent',
    'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/60 rounded-xl ' => !$user,
    'header-theme-btn text-accent' => $user
    ])>
        <i class="fa-solid fa-bell text-lg"></i>
        <!-- Pulse Indicator Badge -->
        <span id="notificationBadge" @class(['hidden absolute flex', ' top-1.5 right-1.5  h-2 w-2' => !$user, ' h-3 w-3 -top-0.5 right-0' => $user])>
            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-accent/70 opacity-75"></span>
            <span @class(['relative inline-flex rounded-full ', 'h-2 w-2 bg-rose-500' => !$user, 'h-3 w-3 bg-accent' => $user])></span>
        </span>
    </button>

    <!-- Dropdown Panel (Bottom Sheet on Mobile, Popover on Desktop) -->
    <div id="notificationMenu"
         class="hidden fixed sm:absolute left-0 sm:left-auto right-0 bottom-0 sm:bottom-auto sm:top-full mt-2 w-full sm:w-96 bg-white dark:bg-slate-900 border-t sm:border border-slate-100 dark:border-slate-800/40 rounded-t-3xl sm:rounded-2xl shadow-2xl z-50 overflow-hidden transition-all animate-slideUp sm:animate-fadeIn">
        ...
        <!-- Header -->
        <div
            class="px-4 py-3 border-b border-slate-100 dark:border-slate-800/40 bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between">
            <div class="flex items-center gap-1.5">
                <h3 class="text-xs font-bold text-slate-800 dark:text-slate-100">Notifications</h3>
                <span id="notificationCountBadge"
                      class="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200/50 dark:border-slate-700/50">0</span>
            </div>
            <button type="button"
                    id="markAllReadBtn" @class(['text-[10px] font-bold bg-transparent border-none outline-none cursor-pointer', ' text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300' => !$user, 'text-accent hover:text-amber-700' => $user])>
                Mark all as read
            </button>
        </div>
        ...
        <div id="notificationList"
             class="max-h-[50vh] sm:max-h-[320px] overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60">
            <!-- Loading State -->
            <div id="notificationLoader" class="p-6 text-center">
                <i @class(['fa-solid fa-circle-notch fa-spin text-lg', 'text-slate-400 dark:text-slate-600' => !$user, 'text-accent' => $user])></i>
            </div>

            <!-- Empty State -->
            <div id="notificationEmptyState" class="hidden p-8 text-center">
                <div
                    class="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800/40 text-slate-400 dark:text-slate-500 flex items-center justify-center mx-auto mb-3">
        ...
        <!-- Footer -->
        <div
            class="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800/40 bg-slate-50/50 dark:bg-slate-900/50 text-center flex-shrink-0">
            <a id="viewAllNotificationsBtn"
               href="#" @class(['text-[10px] font-bold bg-transparent border-none outline-none cursor-pointer', ' text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300' => !$user, 'text-accent hover:text-amber-700' => $user])>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const isUserSide = {{ $user ? 'true' : 'false' }};
        ...
        let baseUrl = '/admin/notifications';
        if (window.location.pathname.startsWith('/seller')) {
            baseUrl = '/seller/notifications';
        } else if (!window.location.pathname.startsWith('/admin')) {
            baseUrl = '/account/notifications-api';
        }

        if (viewAllBtn) {
            viewAllBtn.href = baseUrl === '/account/notifications-api' ? '/account/notifications' : `${baseUrl}/all`;
        }
        ...
        function createNotificationItem(item) {
            ...
            else if (item.type === 'order') colorClasses = isUserSide ? 'bg-accent/10 text-accent' : 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-450';
            ...
            row.innerHTML = `
                <div class="w-8 h-8 rounded-lg ${colorClasses} flex items-center justify-center flex-shrink-0 relative">
                    ...
                    ${isUnread ? `<span class="absolute -top-0.5 -right-0.5 w-2 h-2 ${isUserSide ? 'bg-accent' : 'bg-indigo-600 dark:bg-indigo-450'} border border-white dark:border-slate-900 rounded-full"></span>` : ''}
                </div>
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between gap-1.5">
                        <p class="${titleClass} truncate">${item.title}</p>
                        ${isUnread ? `<span class="px-1.5 py-0.5 text-[8px] font-bold ${isUserSide ? 'text-accent bg-accent/10' : 'text-indigo-600 bg-indigo-100/60 dark:text-indigo-400 dark:bg-indigo-950/60'} rounded flex-shrink-0">New</span>` : ''}
                    </div>
