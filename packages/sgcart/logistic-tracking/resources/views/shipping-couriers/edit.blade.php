...
@section('title', 'Edit Shipping Courier — SGCart Admin')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="font-display text-2xl font-bold">Edit Shipping Courier</h1>
        <p class="text-sm text-slate-400 mt-0.5">Admin / Logistics / Shipping Couriers / Edit</p>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm w-full overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <h2 class="font-semibold text-sm">Edit Courier Details</h2>
    </div>
    
    <form action="{{ route('admin.couriers.update', $courier->id) }}" method="POST" class="p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $courier->name) }}" required
                class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg py-2.5 px-3.5 text-sm placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-slate-850 dark:text-slate-100 @error('name') border-rose-500 focus:border-rose-500 focus:ring-rose-500 @enderror"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="url" name="url" id="url" value="{{ old('url', $courier->url) }}"
                    class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg py-2.5 px-3.5 text-sm placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-slate-850 dark:text-slate-100 @error('url') border-rose-500 focus:border-rose-500 focus:ring-rose-500 @enderror"
                    placeholder="https://...">
                @error('url')
                    <p class="text-rose-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="email" name="support_email" id="support_email" value="{{ old('support_email', $courier->support_email) }}"
                    class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg py-2.5 px-3.5 text-sm placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-slate-850 dark:text-slate-100 @error('support_email') border-rose-500 focus:border-rose-500 focus:ring-rose-500 @enderror"
