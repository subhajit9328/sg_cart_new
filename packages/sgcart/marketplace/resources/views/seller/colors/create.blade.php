...
@section('title', 'Add Color Swatch — Seller Portal')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="font-display text-2xl font-bold text-slate-900 dark:text-white">Add Color Swatch</h1>
        <p class="text-slate-500 dark:text-slate-400 text-xs mt-1">Create a new custom color for your product variants.</p>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm w-full overflow-hidden max-w-2xl">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <h2 class="font-semibold text-sm text-slate-700 dark:text-slate-200">Enter Swatch Details</h2>
    </div>

    <form method="POST" action="{{ route('seller.colors.store') }}" class="p-6 space-y-6">
        @csrf
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="hex_code" id="hex_code" value="{{ old('hex_code', '000000') }}" required
                    ...
                    <div class="w-11 h-11 rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden relative shadow-sm">
                        <input type="color" id="colorPicker" value="#000000" class="absolute inset-0 w-full h-full border-0 cursor-pointer p-0">
                    </div>
                </div>
