...
@section('title', 'Edit Color Swatch — Seller Portal')

@section('content')
<!-- Page Header -->
<div class="flex items-center gap-3 mb-6">
    <a href="{{ route('seller.colors.index') }}" class="w-10 h-10 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-center text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors no-underline">
        <i class="fa-solid fa-arrow-left text-xs"></i>
    </a>
    <div>
        <h1 class="font-display text-2xl font-bold text-slate-900 dark:text-white">Edit Color Swatch</h1>
        <p class="text-slate-500 dark:text-slate-400 text-xs mt-1">Modify your custom color swatch details.</p>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm w-full overflow-hidden max-w-2xl">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <h2 class="font-semibold text-sm text-slate-700 dark:text-slate-200">Edit Swatch Details</h2>
    </div>

    <form method="POST" action="{{ route('seller.colors.update', $color->id) }}" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $color->name) }}" required autofocus
                ...
                <div class="flex gap-3">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="hex_code" id="hex_code" value="{{ old('hex_code', str_replace('#', '', $color->hex_code)) }}" required
                            class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg py-2.5 pl-8 pr-3.5 text-sm placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-slate-800 dark:text-slate-100 @error('hex_code') border-rose-500 @enderror"
                            placeholder="e.g. 000000">
                    </div>
                    @php
                        $hexValue = str_starts_with($color->hex_code, '#') ? $color->hex_code : '#' . $color->hex_code;
                    @endphp
                    <div class="w-11 h-11 rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden relative shadow-sm">
                        <input type="color" id="colorPicker" value="{{ $hexValue }}" class="absolute inset-0 w-full h-full border-0 cursor-pointer p-0">
                    </div>
                </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
            <a href="{{ route('seller.colors.index') }}" class="px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-sm font-medium transition-colors text-slate-700 dark:text-slate-300 no-underline">
                Cancel
            </a>
