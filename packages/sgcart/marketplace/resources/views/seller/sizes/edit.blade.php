...
@section('title', 'Edit Size Label — Seller Portal')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="font-display text-2xl font-bold text-slate-900 dark:text-white">Edit Size Label</h1>
        <p class="text-slate-500 dark:text-slate-400 text-xs mt-1">Modify your custom size swatch details.</p>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm w-full overflow-hidden max-w-2xl">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <h2 class="font-semibold text-sm text-slate-700 dark:text-slate-200">Edit Size Details</h2>
    </div>

    <form method="POST" action="{{ route('seller.sizes.update', $size->id) }}" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $size->name) }}" required autofocus
                    class="w-full bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 rounded-lg py-2.5 px-3.5 text-sm placeholder:text-slate-400 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-slate-800 dark:text-slate-100 @error('name') border-rose-500 focus:border-rose-500 @enderror"
