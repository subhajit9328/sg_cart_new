            ...
            <div class="flex items-center gap-2 mt-2">
                ...
                @if($seller->status->value === 'pending')
                    <form id="approve-detail-form" action="{{ route('admin.sellers.approve', $seller) }}" method="POST">
                        @csrf
                        <button type="button" onclick="confirmApproveDetail()" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-1 px-2.5 rounded text-[10px] uppercase transition-colors">Approve</button>
                    </form>
                @elseif($seller->status->value === 'approved')
                    <form id="suspend-detail-form" action="{{ route('admin.sellers.suspend', $seller) }}" method="POST">
                        ...
                        <button type="button" onclick="confirmSuspendDetail()" class="bg-rose-600 hover:bg-rose-700 text-white font-bold py-1 px-2.5 rounded text-[10px] uppercase transition-colors">Suspend</button>
                    </form>
                @endif
            </div>
            @if($seller->status->value === 'suspended' && $seller->suspension_reason)
                <div class="mt-3 p-3 bg-rose-50 dark:bg-rose-950/15 border border-rose-100 dark:border-rose-900/30 rounded-lg text-xs text-left">
                    <span class="font-bold text-rose-700 dark:text-rose-400 block mb-0.5">Suspension Reason:</span>
                    <p class="text-slate-650 dark:text-slate-405 leading-relaxed m-0">{{ $seller->suspension_reason }}</p>
                </div>
            @endif
        </div>
    </div>
