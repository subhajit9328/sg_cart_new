                ...
                <div class="inline-flex gap-1.5 justify-end">
                    ...
                    @if($seller->status->value === 'pending')
                        <form id="approve-form-{{ $seller->id }}" action="{{ route('admin.sellers.approve', $seller) }}" method="POST" class="inline">
                            @csrf
                            <button type="button" onclick="confirmApprove({{ $seller->id }})" class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 flex items-center justify-center transition-colors cursor-pointer text-emerald-600" title="Approve Seller">
