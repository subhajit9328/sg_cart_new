...
@section('content')
    ...
    @php
        $headers = [
            ...
            ['label' => 'Charge / Rate', 'key' => 'cost', 'sortable' => true],
            ['label' => 'Min. Cart Subtotal', 'key' => 'min_order_amount', 'sortable' => true],
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start mb-6">
        ...
        <x-data-table
            ...
            tableId="shippingRatesTableWrapper"
            searchInputId="searchRatesInput"
            totalCountId="shippingRatesTotalCount"
            :items="$rates"
            :headers="$headers"
        >
            @forelse($rates as $rate)
                <tr>
                    ...
                    <td class="px-5 py-4 whitespace-nowrap">
                        @if($rate->is_active)
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400">
                            Active
                        </span>
                        @else
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400">
                            Disabled
                        </span>
                        @endif
                    </td>
                    <td class="px-5 py-4 text-right whitespace-nowrap">
                        <div class="inline-flex gap-1.5">
                            <a href="{{ route('admin.shipping.edit', $rate->id) }}"
                               class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center justify-center transition-colors"
                               title="Edit Shipping Rate">
                                <i class="fa-solid fa-pen text-slate-500 dark:text-slate-400 text-xs"></i>
                            </a>

                            <button onclick="openDeleteModal('{{ $rate->id }}', '{{ $rate->name }}')"
                                    class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-rose-50 dark:hover:bg-rose-500/10 flex items-center justify-center transition-colors"
        ...
        <div
            ...
            <div
                class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex items-center gap-2">
            ...
            <form action="{{ route('admin.shipping.settings.update') }}" method="POST" class="p-5 space-y-4">
                ...
                <div class="space-y-3">
                    ...
                    <div
                        class="border rounded-xl p-3.5 cursor-pointer hover:border-slate-300 dark:hover:border-slate-700 transition-all relative overflow-hidden config-card {{ $selectionMode === 'user_choice' ? 'border-blue-600 dark:border-blue-500 bg-blue-50/10 dark:bg-blue-500/5 ring-1 ring-blue-500' : 'border-slate-200 dark:border-slate-800 bg-transparent' }}"
                        onclick="toggleConfig(this)">
                        <div class="flex items-start gap-3">
                            <input type="radio" name="shipping_selection_mode" value="user_choice"
                                   {{ $selectionMode === 'user_choice' ? 'checked' : '' }} class="mt-0.5 focus:ring-blue-500 h-4 w-4 text-blue-600 border-slate-300 dark:border-slate-700 cursor-pointer">
                            <div class="flex-1">
                                <span
                                    class="block text-xs font-bold text-slate-800 dark:text-slate-200">Customer Choice</span>
                    ...
                    <div
                        class="border rounded-xl p-3.5 cursor-pointer hover:border-slate-300 dark:hover:border-slate-700 transition-all relative overflow-hidden config-card {{ $selectionMode === 'auto_cheapest' ? 'border-blue-600 dark:border-blue-500 bg-blue-50/10 dark:bg-blue-500/5 ring-1 ring-blue-500' : 'border-slate-200 dark:border-slate-800 bg-transparent' }}"
                        onclick="toggleConfig(this)">
                        <div class="flex items-start gap-3">
                            <input type="radio" name="shipping_selection_mode" value="auto_cheapest"
                                   {{ $selectionMode === 'auto_cheapest' ? 'checked' : '' }} class="mt-0.5 focus:ring-blue-500 h-4 w-4 text-blue-600 border-slate-300 dark:border-slate-700 cursor-pointer">
                            <div class="flex-1">
                ...
                <button type="submit"
                        class="w-full py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs font-semibold shadow-lg shadow-blue-600/10 hover:shadow-blue-600/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer">
