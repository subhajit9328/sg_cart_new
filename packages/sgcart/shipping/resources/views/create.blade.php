...
@section('title', 'Create Shipping Rate — SGCart Admin')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="font-display text-2xl font-bold">Add Shipping Rate</h1>
        <p class="text-sm text-slate-400 mt-0.5">Admin / Logistics / Shipping Rates / Add</p>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm w-full overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <h2 class="font-semibold text-sm">Enter Shipping Rate Details</h2>
    </div>
    
    <form action="{{ route('admin.shipping.store') }}" method="POST" class="p-6 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" name="cost" id="cost" value="{{ old('cost') }}" required
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" name="min_order_amount" id="min_order_amount" value="{{ old('min_order_amount', '0.00') }}"
