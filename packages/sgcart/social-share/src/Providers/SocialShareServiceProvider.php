...
namespace SGCart\SocialShare\Providers;
...
class SocialShareServiceProvider extends ServiceProvider
{
    ...
    protected function autoInstall(): void
    {
        try {
            ...
            // Check database connection and verify if social_posts table is missing
            if (Schema::connection(null)->getConnection()->getPdo() && !Schema::hasTable('social_posts')) {
                // Programmatically trigger package database migrations
                Artisan::call('migrate', [
                    '--path' => 'packages/sgcart/social-share/database/migrations',
                    '--force' => true
                ]);
            }
            ...
            if (class_exists(\Spatie\Permission\Models\Permission::class)) {
                $permission = \Spatie\Permission\Models\Permission::firstOrCreate(['name' => 'manage social shares', 'guard_name' => 'web']);
