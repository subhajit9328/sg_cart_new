...
@section('title', 'Edit Coupon — SGCart Admin')
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm w-full overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <h2 class="font-semibold text-sm">Update Coupon Details</h2>
    </div>
    
    <form action="{{ route('admin.coupons.update', $coupon->id) }}" method="POST" class="p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0.01" name="value" id="value" value="{{ old('value', $coupon->value) }}" required
        ...
        <div>
            <label class="flex items-center gap-3 cursor-pointer select-none py-1">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $coupon->is_active ? '1' : '0') == '1' ? 'checked' : '' }}
